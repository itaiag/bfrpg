...
- הזינו את גודל רשת המשבצות כדי ליצור את משטח הציור (שימו לב שפעולה זו מוחקת כל מה שציירתם קודם)
- לחצו או גררו כדי ליצור חדרים ומסדרונות. הלחיצה מחליפה בין משבצות לבנות לכחולות.
- לחצו על משבצת ואז הקלידו תו במקלדת כדי לסמן משבצת באות או מספר (ניתן להקליד מספר תווים).
- לחצו לחיצה ימנית על משבצת כדי למחוק טקסט קיים.



```{ojs}
viewof gridsize = Inputs.text({placeholder:"גודל רשת"})

viewof grid1 = Inputs.button("צור רשת", {reduce: () =>  draw(Number(gridsize))})
...
viewof form = Inputs.form(
  {
    button1: "  ",
    button2: Inputs.button("מחק רשת", {reduce: () => resetGrid()})
  },
  {
...
```{ojs}
Inputs.button("ייצא ל-PNG", {reduce: () => exportToImage(size, "png", Number(gridsize), grid1.col, grid1.chars)})
```
