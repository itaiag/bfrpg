...
function savetoFile(){
    ...
    // Both are <textarea>, so the typed text lives in the value property,
    // not in the element's text content.
    char.Background = d3.select('#background').property('value')
    char.MiscNotes = d3.select('#miscNotes').property('value')
