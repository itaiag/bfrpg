  ...
  <h2>פרטי דמות</h3>
    ...
    <div class="grid align-items-center mx-1 justify-content-between" style="--bs-gap:0.5rem">
      ...
      <div class=" g-col-12 g-col-md-7 ">
          ...
          <div class="mb-5">
            <input id="Name" type="text" placeholder="שם דמות" autocomplete="off" >
          </div>
          <div>
            <dl class="field">
              <div id="charClass">
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  <li class="nav-item" role="presentation">
    <button class="nav-link active" id="attributes-tab" data-bs-toggle="tab" data-bs-target="#attributes" type="button" role="tab" aria-controls="attributes" aria-selected="true">תכונות</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="combat-tab" data-bs-toggle="tab" data-bs-target="#combat" type="button" role="tab" aria-controls="combat" aria-selected="false">קרב</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="Spells-tab" data-bs-toggle="tab" data-bs-target="#spells" type="button" role="tab" aria-controls="spells" aria-selected="false">כישופים</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="equipment-tab" data-bs-toggle="tab" data-bs-target="#equipment" type="button" role="tab" aria-controls="equipment" aria-selected="false">ציוד</button>
  </li>
  <li class="nav-item" role="presentation">
        <button class="nav-link" id="notes-tab" data-bs-toggle="tab" data-bs-target="#notes" type="button" role="tab" aria-controls="notes" aria-selected="false">הערות</button>
  </li>
</ul>
    ...
    <div class="grid mx-2 my-2" >
      ...
      <div class="g-col-12 g-col-md-6">
          ...
          <div id="DRP">
              ...
              <label for="save-drpBox">קרן מוות, רעל</label>
          </div>
          <div id="Wand">
              <input autocomplete="off" type="text" id="save-wandBox">
              <label for="save-wandBox">שרביטי קסם</label>
          </div>
          <div id="Par">
              <input autocomplete="off" type="text" id="save-parBox" >
              <label for="save-parBox">שיתוק / התאבנות</label>
          </div>
          <div id="DragonBreath">
              <input autocomplete="off" type="text" id="save-dbBox">
              <label for="save-dbBox">נשיפת דרקון</label>
          </div>
          <div id="Spells">
              <input autocomplete="off" type="text" id="save-spellBox" >
              <label for="save-spellBox">כישופים</label>
          </div>
          </div>
      </div>
    </div> <!-- big grid -->
             ...
             <div class="container mt-4 ">
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text">פתיחת מנעולים</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text">נטרול מלכודות</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text ">כייסות</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text ">תנועה חרישית</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text ">טיפוס על קירות</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text ">התחבאות</span>
                    </div>
                ...
                <div class="input-group mb-3 justify-content-end">
                    <div class="input-group-prepend">
                        <span class="input-group-text ">הקשבה</span>
                    </div>
            ...
            <h2>שפות</h2>
             <div class="container mt-4 me-5">
                ...
                <div class="input-group mb-2 rounded">
                    <input autocomplete="off" type="text" class="form-control rounded lang" placeholder="שפה 1" id="language1">
                </div>
                </div>
                <button id="addInputButton" class="btn btn-primary rounded">הוסף שפה</button>
            </div>
        ...
        <div class="g-col-12 g-col-md-4">
            <h2>יכולות אחרות</h2>
             <div class="container mt-4 me-5">
                ...
                <div class="input-group mb-2 rounded">
                    <input autocomplete="off" type="text" class="form-control rounded oskill" id="skill1" placeholder="יכולת 1" >
                </div>
                </div>
                <button id="addSkillButton" class="btn btn-primary rounded">הוסף יכולת</button>
            </div>
        </div>
...
<style>
      ...
      div.health label {
          ...
          font-weight: 800;
          flex: 1 1 auto;
          /* RTL: keep the label clear of the panel's right border */
          text-align: right;
...
// Row ids have to stay unique even after rows are deleted, so keep a
// monotonic counter on the container rather than counting the rows present.
function nextRowId (container) {
    const node = d3.select(container).node()
    const next = (Number(node.dataset.rowSeq) || 1) + 1
    node.dataset.rowSeq = String(next)
    return next
}
...
function addInputField() {
      ...
      var idNum = nextRowId('#inputContainer');
...
function addSkill() {
      ...
      var inputCount = inputContainer.selectAll('.input-group').size();
      var idNum = nextRowId('#inputContainerSkills');
      ...
      inputGroup
        ...
        .attr('placeholder', 'יכולת ' + (inputCount + 1));

      inputGroup
        .append('button')
        .attr('class', 'btn btn-danger delete-button rounded-end')
        .attr('type', 'button')
        .attr('id', 'skillDel' + idNum)
        .text('-')
        .on('click', function() {
          inputGroup.remove();
        });
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.4/d3.js" integrity="sha512-51xEUbZobtzTG53DuETlzoOZk9cWBiEXV/5Mh7m1htisZLZS4pIHP29J/61/t47rL5Yo0ZCzaeaKLabxdurSJg==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
  ...
  <div class="container">
    <button id="downloadCharButton" class="btn btn-primary">הורד דמות</button>
  </div>
```
